...
Asymmetric keys are generated such as $(PKA_o, PKB_o)=GenAsym(RSA, l)$,
where $PKA_o$ is the private key, $PKB_o$ the public key, and $l$ is the length.

The master of the organization is a normal user (not sub-user), and gets the
key of the organization encrypted thanks to its $PKB_o$.
...
\begin{itemize}
	\item $CEK_u$: sub-user's CEK
	\item $KEK_u$: sub-user's KEK
	\item $PKA_o$: asymmetric private key of the organization
	\item $PKB_o$: asymmetric public key of the organization
	\item $A$: $A=AsymEnc(PKB_o, CEK_u)$
	\item $B$: $B=Enc(KEK_u, CEK_u)$
\end{itemize}
...
The admin gets the sub-user's $PKB_u$ and encrypts $PKA_o$ with it such as
$PKA_o_e=AsymEnc(PKB_u,PKA_o)$. Afterwards, $PKA_o_e$ is stored on
the server.
...
$packet=(B||S||IV||AD)$. We encrypt it with $PKB_o$ such as
$A=AsymEnc(PKB_o, CEK_u)$ and store it as well.
